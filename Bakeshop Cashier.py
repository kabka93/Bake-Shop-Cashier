...
class main:
    ...
    def login(self):
        ...
        if result:
            ...
            ms.showerror('Woopsidaisies!',' Entered Username Not Found.')
    ...
    def new_user(self):
        ...
        if c.fetchall():
            ms.showerror('Error!','Username or Password Taken Try a Diffrent One.')
        else:
...
root.title("Bakeshop Cashier")

Tops = Frame (root, width = 1350 , height=100, bd= 6, relief="raise")
Tops.pack(side=TOP)
lblTitle = Label(Tops, fg ='deep pink',  background = 'SteelBlue1',  font=('times new roman' ,  50,  'bold','italic'), text="\tBakeshop Cashier\t\t\t")
...
def iExit():
        qExit= messagebox.askyesno("Bakeshop", "Do you want to quit the cashier?")
        if qExit  >  0:
            root.destroy()
            return
